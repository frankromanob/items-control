...
const ProductAdminPage: FC<Props> = ({ product }) => {
    ...
    const onChangeSize = (size: string) => {
        ...
        setValue('sizes', [...currentSizes as any, size], { shouldValidate: true })
    }
    ...
    const onNewTag = (tag: string) => {
        ...
        setValue('tags', [...currentTags as any, tag.trim().toLowerCase()], { shouldValidate: true })
    }
    ...
    const onSubmit = async (form: formData) => {
        ...
        if (form.images.length < 2) { return alert('Se requiere al menos 2 imagenes') }
        setIsSaving(true)

        try {
            const respuesta = await tesloApi({
                url: '/admin/products',
            ...
            if (respuesta) {
                //TODO Reload
                router.replace(`/admin/products/${respuesta.data.slug}`)
            }
